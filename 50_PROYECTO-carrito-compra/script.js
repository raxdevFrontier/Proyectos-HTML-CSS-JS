...
function cargarEventListeners() {
    ...
    vaciarCarrito.addEventListener('click', () => {
        //se resetea el array del carrito que contiene los cursos
        articulosCarrito = []

        //se elimina todo el HTML del carrito
        limpiarHTML()
    })

    document.addEventListener('DOMContentLoaded', () => {
        articulosCarrito = JSON.parse(localStorage.getItem('carrito')) || []
        /* Recuperamos los datos dl local storage, 
        y en caso de que no haya datos asigne un array vacio a 'articulosCarrito' 
        para asegurar que la funcion 'carritoHTML()' funcione y no dé error 
        ( más explicación en "53_PROYECTO-almacenar-en-localStorage" )*/
...
function eliminarCurso(e){
    ...
    if(e.target.classList.contains('borrar-curso')){
        ...
        //Eliminar el curso seleccionado, mediante el ID de éste (cursoId)
        // Mediante .filter() iteramos el array de 'articulosCarrito' para que 
        // nos devuelva los articulos con un ID diferente al del curso que se esta eliminando
        
        articulosCarrito = articulosCarrito.filter( curso => curso.id !== cursoId)

        carritoHTML() // Se itera sobre el carrito, y se vuelve a cargar/mostrar el HTML
    }
}
